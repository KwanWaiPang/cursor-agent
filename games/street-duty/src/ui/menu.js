...
export class PauseMenu {
  constructor(parent, ctx) {
    ...
    // ---- sensitivity -----------------------------------------------------
    this.sens = this._slider('鼠标灵敏度', 0.2, 3.0, 0.01, (v) => {
    ...
    // ---- field of view ---------------------------------------------------
    this.fov = this._slider('视野 FOV', 65, 120, 1, (v) => {
    ...
    this.resumeBtn = el('button', 'ow-btn primary', btns, '继续');
    this.resumeBtn.type = 'button';
    this.resumeBtn.addEventListener('click', () => this.close());
    const reset = el('button', 'ow-btn', btns, '默认');
    reset.type = 'button';
    ...
    const hub = el('a', 'ow-btn', btns, '返回游戏馆');
    hub.href = '../../';
    setStyle(hub, 'text-decoration', 'none');
    setStyle(hub, 'display', 'inline-flex');
    setStyle(hub, 'align-items', 'center');
    setStyle(hub, 'justify-content', 'center');
    el('div', 'hint', inner, 'ESC 继续 · WASD 移动 · SHIFT 冲刺 · R 换弹 · F 互动');
