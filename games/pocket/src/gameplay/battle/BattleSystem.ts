...
export class BattleSystem implements System {
  ...
  private async intro(): Promise<void> {
    ...
    const sendSay = this.ui.say(`去吧，${b.player.name}！`, 0.5);
  ...
  private async playEvent(ev: BattleEvent): Promise<void> {
    switch (ev.kind) {
      ...
      case 'faint':
        ...
        break;
      case 'run':
        await this.ui.say(ev.success ? '安全地逃走了！' : '逃不掉！', 0.8);
        break;
      case 'end':
        break;
    }
  ...
  private async playMove(ev: Extract<BattleEvent, { kind: 'move' }>): Promise<void> {
    ...
    const sayP = this.ui.say(`${attackerName} 使用了 ${move.name}！`, 0.4);
    ...
    if (ev.missed) {
      ...
      if (ev.crit) await this.ui.say('会心一击！', 0.55);
      if (eff >= 2) {
  ...
  private async playStat(ev: Extract<BattleEvent, { kind: 'stat' }>): Promise<void> {
    ...
    await sayP;
    if (ev.failed) {
      await this.ui.say('但是失败了！', 0.6);
    } else {
      const statName =
        ev.stat === 'atk' ? '攻击' : ev.stat === 'def' ? '防御' : ev.stat === 'spe' ? '速度' : '命中率';
      await this.ui.say(`${targetName} 的 ${statName} ${ev.delta < 0 ? '下降了' : '提升了'}！`, 0.7);
    }
  ...
  private async playFaint(side: Side): Promise<void> {
    ...
    await this.ui.say(`${name} 倒下了！`, 0.85);
  }
